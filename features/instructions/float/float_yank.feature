Feature: float_yank instruction
  ...
  Scenario: positive pointer less than stack depth -> pull up item at depth N to the top
    ...
    When I execute the Nudge instruction "float_yank"
    Then stack :float should have depth 5
    And the :float stack should be ["1.1", "2.2", "4.4", "5.5", "3.3"]
  ...
  Scenario: large positive pointer -> pull up the bottom item of the stack
    ...
    When I execute the Nudge instruction "float_yank"
    Then stack :float should have depth 5
    And the :float stack should be ["2.2", "3.3", "4.4", "5.5", "1.1"]
