...
Feature: Iteration control structures
  ...
  Scenario: code_do_count should build a macro that will execute the :code item n times
    ...
    When I execute "do code_do_count"
    Then a new :exec item "block { value «int» do exec_do_count block {ref x}}\n«int» 8"
  ...
  Scenario: code_do_count stops building its macro when n==1
    ...
    When I execute "do code_do_count"
    Then a new :exec item "block {ref x}"
    And the arguments will have disappeared
  ...
  Scenario: code_do_count creates an :error for a negative count
    ...
    And "-9" on the :int stack
    When I execute "do code_do_count"
    Then a new :error item "code_do_count called with a negative count"
    And the arguments will have disappeared
    
    
  Scenario: code_do_count does not push a counter value
    Given an interpreter with "block {ref x}" on the :code stack
    And "12" on the :int stack
    When I execute "do code_do_count"
  ...
  Scenario: code_do_range should execute the top :code item for each integer in [n1, n2]
    ...
    When I execute "do code_do_range"
    Then a new :exec item "block { value «int» value «int» do exec_do_range block {ref x}}\n«int» 10\n«int» 11"
    And the arguments will have disappeared
  ...
  Scenario: code_do_range should just replicate the code arg when the indices are identical
    ...
    When I execute "do code_do_range"
    Then I'll see a new :exec item "block {ref x}"
    And the arguments will have disappeared
  ...
  Scenario: code_do_range pushes its counter value to :int
    Given an interpreter with "block {ref x}" on the :code stack
    And "19" on the :int stack
    When I execute "do code_do_range"
    Then the :int stack will contain "19"
